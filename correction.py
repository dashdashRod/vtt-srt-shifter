    ...
    for x in final:
        if(x.__contains__('-->')):
            ...
            if(len(result[0].split(':')) == 3):
                ...
                x = x.replace(result[0],datetime.strftime(final2[0],'%M:%S.%f')[:-3])
                x = x.replace(result[1],datetime.strftime(final2[1],'%M:%S.%f')[:-3]) 
                final[counter] = x
        counter += 1
...
if __name__ == '__main__':
    if(splitext(nome_arquivo)[1] == '.vtt'):
        ...
        temp = False
    funcao_positiva(nome_arquivo=nome_arquivo,segundos=19,operator=True)
